...
protocol ResetPasswordModuleOutput {
    func commmitEmail(email: String)
    func changePasswordSuccess()
    func goToLogin()
}

class ResetPasswordFlowCoordinator: ResetPasswordModuleOutput, CodeVerificationSuccessProtocol, FlowCoordinator {
    func commmitEmail(email: String) {
    ...
    func changePasswordSuccess() {
        let changePasswordSuccesView = ResetPasswordSuccessView(successButtonPressed: goToLogin)
