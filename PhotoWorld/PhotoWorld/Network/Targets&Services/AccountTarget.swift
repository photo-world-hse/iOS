...
extension AccountEndpoints: TargetType {
    ...
    var method: Moya.Method {
        switch self {
        case .getTags, .getServices, .getAccountInfo, .getUserAccountInfo, .getUserProfiles:
            return .get
        case .createAlbum, .createAccount, .createComment:
            return .put
        }
    }
    ...
    var task: Task {
        switch self {
        case .getTags, .getServices, .getUserAccountInfo, .getAccountInfo, .getUserProfiles:
            return .requestPlain
    ...
    var headers: [String : String]? {
        ...
        case .getTags(_, let token), .getServices(_, let token), .getUserProfiles(token: let token),
                .createAlbum(_, _, let token), .createAccount(_, let token), .createComment(_, _, _, token: let token), .getUserAccountInfo(_, token: let token), .getAccountInfo(_, _, token: let token):
